...
Создать .cpp файл:
            #include "jni.h"
            #include "Native_NativeDemo.h" //имя хедера сгенерированнго javah

            //добавить перед каждым методом, который попадает в DLL (необязательно)
            __declspec( dllexport ) 
            JNIEXPORT jint JNICALL Java_NativeDemo_gcd(JNIEnv* env, jobject obj, jint p, jint q)
            {
                 //такой прототип берется из хедера, "Native_NativeDemo.h";
...
2) используя VisualC (64 bit):
   запустить командную строку разработчика VC(x64)(или Native Tools(x64) для VS) и создать DLL командой:
    > Cl /LD myCprog.cpp
...
теперь как обычно запускается класс(с нативным методом):
    > java -classpath classes Native.NativeDemo